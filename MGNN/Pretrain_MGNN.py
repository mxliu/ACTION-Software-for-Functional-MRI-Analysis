...
class SimSiam(nn.Module):
    ...
    def __init__(self, dim, pred_dim):
        ...
        self.first_encoder = Module_1( input_dim=116,
            hidden_dim=dim,
            #num_classes=2,
            num_heads=2,
            num_layers=2,
    ...
    #def forward(self, x1, x2, A1, A2):
        ...
        """
        z1,  reconstruct_loss1,modularityloss1 = self.first_encoder(data1)  # 16,104,64# NxC
        z2, reconstruct_loss2,modularityloss2 = self.first_encoder(data2)  # (nbatch,64,1,1) # NxC#
       # print(z2.shape)
#        z1 = reduce(z1, 'b n c ->b c', 'mean')
     #   z2 = reduce(z2, 'b n c ->b c', 'mean')
        ...
        reconstruct_loss=reconstruct_loss1+reconstruct_loss2

        modularityloss = modularityloss1 + modularityloss2

        return p1, p2, z1.detach(), z2.detach(),reconstruct_loss, modularityloss
...
def train(train_loader, Model, criterion, optimizer, epoch, args):
    ...
    for i, (data1,data2) in enumerate(train_loader):
        ...
        p1, p2, z1, z2,reconstruct_loss, modularityloss = Model(data1, data2)
        loss = -(criterion(p1, z2).mean() + criterion(p2, z1).mean()) * 0.5+0.01*reconstruct_loss+ 0.01*modularityloss
...
for epoch in range(args.start_epoch, args.epochs):
    ...
    if (epoch+1) % 10 == 0:
        ...
        }, is_best=False, filename='1Pretrained_{}_{:03d}.pth.tar'.format(args.encoder_type,epoch))
