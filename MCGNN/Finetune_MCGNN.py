...
parser.add_argument('--batchsize', default=4, type=int, metavar='N',
...
parser.add_argument('--encoder-type',
                    choices=['MGNN'],
                    default='MGNN',  # Change the default to 'gat'
...
class Model(nn.Module):
    ...
    def __init__(self, dim, pred_dim):
        ...
        self.encoder = Module_1(input_dim=116,
            hidden_dim=dim,
            #num_classes=2,
            num_heads=2,
            num_layers=2,
        ...
        #self.predictor.add_module('BN', nn.BatchNorm1d(pred_dim, track_running_stats=True)),
    ...
    def forward(self, data):
        ...
        """
        #HERE,make ensure that the length of data is consistent with augmented signal:153
       # data=data[:,-153:,:]
   #     print(data.shape)
        z,reconstruct_loss, modularityloss = self.encoder(data)
    #    print(z.shape)
      ...
      #  print(output .shape)
        return z, output,reconstruct_loss, modularityloss
...
for fold, (train_idx, test_idx) in enumerate(kfold.split(full_dataset)):
   ...
   # model.predictor.BN.bias.data.zero_()
    ...
    pretrained='Pretrained_MGNN_019.pth.tar'#'Pretrained_{}_001.pth.tar'.format(args.encoder_type)#
    if pretrained:
        if os.path.isfile(pretrained):
            ...
            assert set(msg.missing_keys) == {"predictor.L1.weight",
                                             "predictor.L2.weight",
                                             "predictor.L2.bias"}
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for i, data in enumerate(training_data_loader):
            ...
            optimizer.zero_grad()
            z, outputs,reconstruct_loss, modularityloss= model(bold)
            batch_loss = criterion(outputs, label.long())+0.01*reconstruct_loss+0.01*modularityloss
        ...
        with torch.no_grad():
            for i, data in enumerate(testing_data_loader):
                ...
                label = label.to(device)
                Labels.append(label)
                z, output ,reconstruct_loss, modularityloss= model(bold)
                batch_loss = criterion(output, label.long())+0.01*reconstruct_loss+0.01*modularityloss
                pre_score = output[:, 1]
