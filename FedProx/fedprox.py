...
from FedProx.options import parse
from FedProx.dataset import Load_Data
from FedProx.model import Module_1, Classifier
from FedProx.weight_avg import W_A
import torch
import numpy as np
...
# train
def step(model, c_model, criterion, data, label, mu=0.0001, g_w=None, device='cpu', optimizer=None, c_optimizer=None):
    # model: encoder, c_model: classifier
    # criterion: loss
    # data: samples, label: labels
    # mu: coefficient of regularization
    # g_w: global parameters
    ...
    # L2 norm of local and global parameters
    parameter = model.state_dict()
    l2_norm = 0
    if g_w is not None:
        for key in parameter.keys():
            gap = (parameter[key]-g_w[key]).norm(2).pow(2)
            l2_norm += gap

    # final loss
    loss = loss + mu/2 * l2_norm
        ...
        for site in range(site_num):
            ...
            for epoch in range(argv.num_epochs):
                ...
                for i, x in enumerate(tqdm(dataloader, ncols=80, desc=f'k:{k},i:{iter_num},s:{site},e:{epoch}')):
                    ...
                    if iter_num == 0: g_w = None
                    else: g_w = parameter_init
