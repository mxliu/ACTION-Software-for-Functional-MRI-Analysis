...
parser.add_argument('--encoder-type',
                    choices=['GCN', 'GIN', 'GraphSAGE','GAT','BrainNetCNN','Transformer','GraphSAGE'],
                    default='GraphSAGE',  # Change the default to 'gat'
...
for fold, (train_idx, test_idx) in enumerate(kfold.split(full_dataset)):
    ...
    model =Model(args.dim, args.pred_dim).double()  # args.pred_dim
    model.to(device)
    print(model)
