...
parser.add_argument('--batchsize', default=8, type=int, metavar='N',
...
parser.add_argument('--encoder-type',
                    choices=['BrainGNN'],
                    default='BrainGNN',  # Change the default to 'gat'
...
full_dataset = ABIDEDataset('/home/qqw/Unsupervised_Pretraining/BrainGNN/ANIHC', 'ABIDE')
...
class Model(nn.Module):
    ...
    def forward(self,data ):
       ...
       # data=data[:,-153:,:]
   #     print(data.shape)
        z,w1, w2, s1, s2  = self.encoder(data.x, data.edge_index, data.batch, data.edge_attr, data.pos)
    #    print(z.shape)
...
for fold, (train_idx, test_idx) in enumerate(kfold.split(full_dataset)):
   ...
   # model.predictor.BN.weight.data.normal_(mean=0.0, std=0.01)
    ...
    pretrained='Pretrained_{}_039.pth.tar'.format(args.encoder_type)
    if pretrained:
...
#            bold, label = data
            ...
            optimizer.zero_grad()
            z,output, w1, w2, s1, s2 = model(data)
            batch_loss = criterion(output, data.y.long())
            _, train_pred = torch.max(output, 1)
            batch_loss.backward()
            optimizer.step()
            train_acc += (train_pred.cpu() == data.y.cpu()).sum().item()
        ...
        with torch.no_grad():
            for i, data in enumerate(testing_data_loader):
                ...
                test_acc += (
                        test_pred.cpu() ==  data.y.cpu()).sum().item()  # get the index of the class with the highest probability
