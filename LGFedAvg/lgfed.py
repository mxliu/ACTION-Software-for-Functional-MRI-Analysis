# Liang, P.P., Liu, T., Ziyin, L., Allen, N.B., Auerbach, R.P., Brent, D., Salakhutdinov, R., Morency, L.P., 2020. Think locally, act globally: Federated learning with local and global representations. arXiv preprint arXiv:2001.01523.

import random
import os
import scipy.io as io
from tqdm import tqdm
from LGLedAvg.options import parse
from LGLedAvg.dataset import Load_Data
from LGLedAvg.model import Module_1, Classifier
from LGLedAvg.weight_avg import W_A
import torch
import numpy as np
...
parameter_init = [model_init.state_dict()]*site_num
parameter_init_fix = [model_init.state_dict()]*site_num
...
for k in range(argv.k_fold):
    ...
    for iter_num in range(argv.num_iters):
        ...
        for site in range(site_num):
            ...
            model.to(device)
            model.load_state_dict(parameter_init[site]) # read server-side parameters
        ...
        for site in range(site_num):
            ...
            model = Module_1(input_dim=116, hidden_dim=64)
            model.to(device)
            model.load_state_dict(parameter_init[site])
            c_model = Classifier(latent_dim=64, num_classes=2)
            c_model.to(device)
